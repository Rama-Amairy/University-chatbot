...
class StartQdrant:
    ...
    def __init__(self):
        ...
        try:
            ...
            self.docker_command = [
                ...
                "-p",
                f"{self.app_setting.PORT}:{self.app_setting.PORT}",
                "-v",
                f"{self.app_setting.VECTOR_DB}:/qdrant/storage",
                "qdrant/qdrant",
            ]
    ...
    def __run_docker_qdrant(self) -> None:
        ...
        try:
            ...
            process = subprocess.Popen(
                self.docker_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE
            )
            ...
            log_info(
                "[QDRANT DOCKER] Container is starting up. Waiting for initialization..."
            )
            time.sleep(6)
        except Exception as e:
    ...
    def __init_qdrant_client(self) -> None:
        ...
        try:
            ...
            log_info(
                f"[QDRANT CLIENT] Connected to Qdrant at {self.app_setting.HOST}:{self.app_setting.PORT}"
            )
        except Exception as e:
    ...
    def create_collection(self, collection_name: str, vector_size: int = 384) -> None:
        ...
        try:
            ...
            log_error(
                f"[QDRANT COLLECTION] Failed to create collection '{collection_name}': {e}"
            )
            raise
    ...
    def insert_embedding(
        ...
        try:
            ...
            log_info(
                f"[QDRANT UPSERT] Inserted point ID {id_} into '{collection_name}'."
            )
        except Exception as e:
    ...
    def search_embeddings(
        ...
        try:
            ...
            log_info(
                f"[QDRANT SEARCH] Found {len(results)} results (requested {top_k})"
            )
            return results
...
if __name__ == "__main__":
    try:
        ...
        qdrant.insert_embedding(
            "embeddings", embedding=embedding, id_=1, payload={"note": "Test embedding"}
        )
