...
from dbs import get_sqlite_engine, init_chunks_table, init_query_response_table
...
async def startup_event():
    ...
    try:
        ...
        log_error(
            f"[STARTUP ERROR] Failed to initialize application: {str(e)}", exc_info=True
        )
        raise
...
app.include_router(
    chunks_embedding_route, prefix="/chunksEmbedding", tags=["Embedding Generation"]
)
app.include_router(chat_route, prefix="/Chatbot", tags=["Chatbot Interaction"])
app.include_router(
    llm_setting_route, prefix="/llmsSettings", tags=["LLM Configuration"]
)
